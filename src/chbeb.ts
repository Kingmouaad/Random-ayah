type TafsirResponse = unknown;

async function aynsfunction() {
  const tafasir: Array<TafsirResponse> = [];
  ...
  for (let i = 0; i < tafsirar.length; i++) {
    const tafsir = await fetch(
      `https://cdn.jsdelivr.net/gh/spa5k/tafsir_api@main/tafsir/${tafsirar[i]}/${numbersurah}/${numberayah}.json`
    );
...
const content = document.querySelector("#chest") as HTMLElement | null;
document.addEventListener("DOMContentLoaded", () => {});
const LOADER_ID = "chest-loader";
function showLoader() {
  if (!content) return;
  let overlay = content.querySelector(`#${LOADER_ID}`) as HTMLElement | null;
  if (!overlay) {
...
function hideLoader() {
  const overlay = content?.querySelector(`#${LOADER_ID}`) as HTMLElement | null;
  if (overlay && overlay.parentElement) {
    overlay.parentElement.removeChild(overlay);
  }
}
const backward = document.querySelector("#backward") as HTMLElement | null;
const forward = document.querySelector("#forward") as HTMLElement | null;
async function handleGenerateClick() {
  try {
    ...
    const tfsir1 = document.createElement("button") as HTMLElement | null;
    const tfsir2 = document.createElement("button") as HTMLElement | null;
    const tfsir3 = document.createElement("button") as HTMLElement | null;
    ...
    if (tfsir?.children) {
      Array.from(tfsir?.children).forEach((childd): any => {
